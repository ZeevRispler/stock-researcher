...
class SynthesisAgent:
    ...
    def __call__(self, state: StockState) -> StockState:
        ...
        guidance = ""
        if hasattr(state, 'needs_retry') and state.needs_retry:
        ...
        if state.query.analysis_type == "single":
            prompt = f"""
            {guidance}
        ...
        if state.query.analysis_type == "comparison" and len(state.query.tickers) == 2:
            t1, t2 = state.query.tickers
            d1, d2 = state.stocks_data[t1], state.stocks_data[t2]

            sentiment1 = d1.get('news_analysis', {}).get('sentiment', 0)
            sentiment2 = d2.get('news_analysis', {}).get('sentiment', 0)
...
═══════════════════════════════════════════════════════════
                    {t1:<10} {t2:<10} Better
───────────────────────────────────────────────────────────
Sentiment Score     {sentiment1:>6.2f}     {sentiment2:>6.2f}     {'→ ' + (t1 if sentiment1 > sentiment2 else t2)}
